...
<t:genericpage>
	...
	<jsp:body>
		<script>
			...
			function downloadApproval(inscriptionId){
				var url = "/SIA/ApprovalCertificate?option=download&inscriptionId=" +  inscriptionId;
				$.get(url, function(data, status){
					data = $(data).find('#pdf-content').html();
					var doc = new jsPDF('p', 'mm', 'a4');
					doc.fromHTML(data);
				    doc.save('CER-APPR-' + new Date() + '.pdf');
				});
			}
			
			function searchStudent(){
				var form = $('#dni-form').serialize()
				var url = "/SIA/ApprovalCertificate?" + form;
							...
							<tbody>
								<c:forEach var="i" items="${inscriptionList}">
									<tr>
										<td>${i.career.name}</td>
										<td>
											<button type="button" class="btn btn-info" onclick="downloadApproval(${i.id});">
